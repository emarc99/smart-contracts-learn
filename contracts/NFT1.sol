...
import "./NFT1Helper.sol";

contract NFTGatedEvent is NFT1Helper {
    // Storage variables
    ...
    function registerForEvent(uint256 _eventId, uint256 _nftId) external {
        ...
        require(ev.nft.balanceOf(msg.sender) > 0, "NFT not detected");

        require(ev.nft.ownerOf(_nftId) == msg.sender, "NFT not detected");
    ...
    function _isManager(uint256 _eventId) internal view {
        ...
        require(msg.sender == events[_eventId].manager, "Sender is not the manager");
